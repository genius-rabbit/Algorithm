...
int main(int argc,char *argv[]){
    ...
    for(i=0;i<NUM;i++){
        ...
        //发送
        //printf("%d\n",n);
        if((int) sendto(socket_a, &send_icmp, ICMP_SIZE, 0, (struct sockaddr*)&to, sizeof(to))==-1){
            printf("sendto error");
            continue;
        }

        //接收
        n= (int) recvfrom(socket_a, buf, BUF_SIZE,0, (struct sockaddr*)&from, (socklen_t *) &fromlen);
        //printf("%d\n",n);   //返回接收数据的字节长度；若无可用数据或对等方已经按序结束，返回0
        if(n==-1){
        ...
        //解包
        if(unpack(buf,n,inet_ntoa(to.sin_addr))==-1){
            printf("unpack error");
        }
...
float timediff(struct timeval *begain, struct timeval *end){
    int n=0;
    n=((end->tv_sec - begain->tv_sec)*1000000 + (end->tv_usec - begain->tv_usec));
    return (float)(n/1000);
}
